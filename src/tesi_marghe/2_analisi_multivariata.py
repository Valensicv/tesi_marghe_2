...
from tesi_marghe.utils.log_reg import run_multivariate_logistic_analysis, run_complete_logistic_analysis
...
# %% Load data
data = ( 
    pl.read_excel("/Users/valensisecarlo/Personal_Projects/tesi_marghe/data/2025-06-28 1119_dati_multicentro.xlsx")
...
for var in all_vars_of_interest:
    ...
    try:
        ...
        univariate_results.append({
            ...
            'Variable_Type': 'CTG' if var in ctg_vars else ('Demographic' if var in demographic_vars else ('Ultrasound' if var in ultrasound_vars else 'Hemodynamic'))
        })
        
        print(f"  OR: {main_result['Odds_Ratio']:.3f} (95% CI: {main_result['OR_CI_Lower']:.3f}-{main_result['OR_CI_Upper']:.3f})")
...
significant_vars = univariate_df.filter(
    ...
    ((pl.col('OR_CI_Lower') > 1) | (pl.col('OR_CI_Upper') < 1))  # OR significantly different from 1
).sort('AUC_p_value')
...
top_auc_vars = univariate_df.filter(
    pl.col('AUC').is_not_null()
).sort('AUC', descending=True).head(10)
...
#  'padc',
'cqvo',
#  'durata_episodi_alta_variazione_norm',
#  'durata_episodi_bassa_variazione_norm',
#  'età',
#  'accelerazioni_norm',
#  'ca',
# 'pfs',
...
try:
    multivariate_results = run_multivariate_logistic_analysis(
        ...
        control_cols=['centro_studi'],
        target_col='outcome_2',
        analysis_name="Multivariate Model vs Outcome 2",
        print_results=True,
        plot_results=True
    )
    ...
    multivariate_results = run_multivariate_logistic_analysis(
        ...
        control_cols=['centro_studi'],
        target_col='outcome_2',
        analysis_name="Multivariate Model (Top 5) vs Outcome 2",
        print_results=True,
        plot_results=True
    )
...
print("Adjusted Odds Ratios (multivariate model):")
for row in multivariate_or.iter_rows(named=True):
...
multivariate_or_sorted = multivariate_or.with_columns(
...
best_univariate = univariate_df.filter(
    pl.col('AUC').is_not_null()
).sort('AUC', descending=True).row(0, named=True)
...
summary_table = pl.DataFrame({
    ...
    'AUC_p_value': [best_univariate['AUC_p_value'], multivariate_results['roc_metrics']['p_value']],
...
univariate_df.write_csv("univariate_screening_outcome2.csv")
print("Univariate results exported to: univariate_screening_outcome2.csv")

# Export multivariate results
multivariate_results['odds_ratios'].write_csv("multivariate_results_outcome2.csv")
print("Multivariate results exported to: multivariate_results_outcome2.csv")

# Export summary
summary_table.write_csv("model_comparison_outcome2.csv")
print("Model comparison exported to: model_comparison_outcome2.csv")
